...
type RemoteReferenceTypeValue = typeof RemoteReferenceType[keyof typeof RemoteReferenceType];

type IncludeShadowTreeType = 'none' | 'open' | 'all';
...
class LocalValue {
   ...
   * @returns {LocalValue} - The created LocalValue object.
   */
  static createSpecialNumberValue(value: string): LocalValue {
   ...
   * @returns {LocalValue} - The created LocalValue object.
   */
  static createDateValue(value: string | Date): LocalValue {
   ...
   * @returns {LocalValue} - The created LocalValue object.
   */
  static createMapValue(map: Record<string, any>): LocalValue {
    let value: any[] = []
   ...
   * @returns {LocalValue} - The created LocalValue object.
   */
  static createObjectValue(object: Record<string, any>): LocalValue {
    let value: any[] = []
   ...
   * @returns {LocalValue} - The created LocalValue object.
   */
  static createRegularExpressionValue(value: string | Record<string, string>): LocalValue {
   ...
   * @returns {LocalValue} - The created LocalValue object.
   */
  static createSetValue(value: Set<any> | any[]): LocalValue {
  ...
  static getArgument(argument: any): LocalValue {
    let localValue = null
    ...
    return localValue as LocalValue
  }
...
class RemoteValue {
  ...
  constructor(remoteValue: RemoteValueData) {
    this.type = null
    this.handle = null
    ...
    if ('type' in remoteValue) {
      ...
      if (PrimitiveType.findByName(typeString as string) != null) {
        this.type = PrimitiveType.findByName(typeString as string)
      } else if (NonPrimitiveType.findByName(typeString as string) != null) {
        this.type = NonPrimitiveType.findByName(typeString as string)
      } else {
        this.type = RemoteType.findByName(typeString as string)
      }
    }

    if ('handle' in remoteValue) {
      this.handle = remoteValue['handle'] as string
    }

    if ('internalId' in remoteValue) {
      this.internalId = remoteValue['internalId'] as string
    }
    ...
    if ('sharedId' in remoteValue) {
      this.sharedId = remoteValue['sharedId'] as string
    }
  ...
  deserializeValue(value: any, type: string | null): any {
...
class ReferenceValue {
   ...
   */
  constructor(handle: RemoteReferenceTypeValue | string, sharedId: string) {
...
class SerializationOptions {
  _maxDomDepth: number;
  _maxObjectDepth: number | null;
  _includeShadowTree: IncludeShadowTreeType;
   ...
   */
  constructor(maxDomDepth: number = 0, maxObjectDepth: number | null = null, includeShadowTree: IncludeShadowTreeType = 'none') {
...
type ResultOwnershipType = 'root' | 'none';
...
class ChannelValue {
  ...
  resultOwnership?: ResultOwnershipType;

  constructor(channel: string, options?: SerializationOptions, resultOwnership?: ResultOwnershipType) {
