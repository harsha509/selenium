...
import { BeforeRequestSent, ResponseStarted, FetchError } from './networkTypes';
...
class Network {
  #callbackId = 0;
  #listener: Map<string, Map<number, EventCallback>>;
  private _driver: WebDriver;
  private _browsingContextIds: string[] | null;
  bidi: any;
  ws: WebSocket;
   ...
   */
  constructor(driver: WebDriver, browsingContextIds: string[] | null) {
    this._driver = driver;
  ...
  addCallback(eventType: string, callback: EventCallback): number {
   ...
   */
  async beforeRequestSent(callback: EventCallback): Promise<number> {
    return await this.subscribeAndHandleEvent('network.beforeRequestSent', callback);
  }
   ...
   * @returns {Promise<void>} - A promise that resolves when the subscription is successful.
   */
  async responseStarted(callback: EventCallback): Promise<number> {
    return await this.subscribeAndHandleEvent('network.responseStarted', callback);
  }
   ...
   * @returns {Promise<void>} - A promise that resolves when the subscription is successful.
   */
  async responseCompleted(callback: EventCallback): Promise<number> {
    return await this.subscribeAndHandleEvent('network.responseCompleted', callback);
  }
   ...
   */
  async authRequired(callback: EventCallback): Promise<number> {
   ...
   * @returns {Promise<void>} - A promise that resolves when the subscription is successful.
   */
  async fetchError(callback: EventCallback): Promise<number> {
    return await this.subscribeAndHandleEvent('network.fetchError', callback);
  }

  async subscribeAndHandleEvent(eventType: string, callback: EventCallback): Promise<number> {
    ...
    this.ws.on('message', (event: WebSocket.Data) => {
      const { params } = JSON.parse(Buffer.from(event.toString()).toString()) as EventData;
      if (params) {
        let response = null;
   ...
   */
  async setCacheBehavior(behavior: CacheBehaviorType, contexts: string[] | null = null): Promise<void> {
...
async function getNetworkInstance(driver: WebDriver, browsingContextIds: string[] | null = null): Promise<Network> {
  let instance = new Network(driver, browsingContextIds);
  await instance.init();
  return instance;
}

export { getNetworkInstance as Network, CacheBehavior, CacheBehaviorType };
