...
import { Message, Source } from './scriptTypes';
import { RealmInfo, RealmType, WindowRealmInfo } from './realmInfo';
import { RemoteValue } from './protocolValue';
...
type TargetType = 'realm' | 'contextTarget';
...
class ScriptManager {
  #callbackId = 0;
  #listener: Map<string, Map<number, CallbackFunction>>;
  ...
  addCallback(eventType: string, callback: CallbackFunction): number {
   ...
   * @returns {Promise<void>} - A promise that resolves when the command is sent.
   */
  async disownBrowsingContextScript(browsingContextId: string, handles: string[], sandbox: string | null = null): Promise<void> {
    const params: any = {
      method: 'script.disown',
      params: {
        handles: handles,
  ...
  async callFunctionInRealm(
    ...
    argumentValueList: LocalValue[] | null = null,
    thisParameter: any = null,
   ...
   */
  async removePreloadScript(script: string): Promise<any> {
  ...
  createEvaluateResult(response: EvaluateResultResponse): EvaluateResultSuccess | EvaluateResultException {
   ...
   */
  async onMessage(callback: CallbackFunction): Promise<number> {
   ...
   * @returns {Promise<void>} - A promise that resolves when the subscription is successful.
   */
  async onRealmCreated(callback: CallbackFunction): Promise<number> {
   ...
   * @returns {Promise<void>} - A promise that resolves when the subscription is successful.
   */
  async onRealmDestroyed(callback: CallbackFunction): Promise<number> {
    return await this.subscribeAndHandleEvent(ScriptEvent.REALM_DESTROYED, callback);
  }

  async subscribeAndHandleEvent(eventType: string, callback: CallbackFunction): Promise<number> {
    ...
    this.ws.on('message', (event: WebSocket.Data) => {
      const { params } = JSON.parse(Buffer.from(event.toString()).toString());
      if (params) {
        let response = null;
...
async function getScriptManagerInstance(browsingContextId: string | string[] | null, driver: WebDriver): Promise<ScriptManager> {
  let instance = new ScriptManager(driver);
  await instance.init(browsingContextId);
  return instance;
}

export default getScriptManagerInstance;
