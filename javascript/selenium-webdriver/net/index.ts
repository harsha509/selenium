...
/**
 * Interface for host and port information.
 */
...
/**
 * Gets the loopback network interface.
 * @return The loopback interface or null if not found.
 */
function getLoInterface(): os.NetworkInterfaceInfo[] | null {
  ...
  return name && os.networkInterfaces()[name] ? os.networkInterfaces()[name] : null;
}
...
function getIPAddress(loopback: boolean, family: string): string | undefined {
  let interfaces: NodeJS.Dict<os.NetworkInterfaceInfo[]> | null;
  if (loopback) {
    const lo = getLoInterface();
    interfaces = lo ? { lo: lo } : null;
  }
  ...
  for (let key in interfaces) {
    ...
    for (let ipAddress of networkInterface) {
 ...
 */
export function getAddress(family: string = 'IPv4'): string | undefined {
 ...
 * @return The IP address or undefined if not available.
 */
export function getLoopbackAddress(family: string = 'IPv4'): string | undefined {
 ...
 * @return A host and port. If no port is present in the argument `hostport`, port is null.
 */
export function splitHostAndPort(hostport: string): HostPort {
