...
export class Zip {
  ...
  private z_: jszip;
  /** @private */
  private pendingAdds_: Set<Promise<any>>;
   ...
   */
  addFile(filePath: string, zipPath: string = path.basename(filePath)): Promise<void> {
    let add = io
      .read(filePath)
      .then((buffer) => this.z_.file(zipPath.replace(/\\/g, '/'), buffer));
   ...
   * @return returns a promise that will resolve when
   * the operation is complete.
   */
  addDir(dirPath: string, zipPath: string = ''): Promise<any> {
    return io.walkDir(dirPath).then((entries) => {
      let archive = this.z_;
      if (zipPath) {
        archive = archive.folder(zipPath) as jszip;
      }
   ...
   * @return a promise that will be resolved with the file's contents as a buffer.
   */
  ...
  /**
   * Gets the internal JSZip instance.
   * @internal
   */
  getJSZip(): jszip {
    return this.z_;
...
export function load(path: string): Promise<Zip> {
  return io.read(path).then((data) => {
    let zip = new Zip();
    return zip.getJSZip().loadAsync(data).then(() => zip);
  });
}
 ...
 * @return a promise that will resolve with `dst` once the archive has been unzipped.
 */
export function unzip(src: string, dst: string): Promise<string> {
  return load(src).then((zip) => {
    ...
    zip.getJSZip().forEach((relPath: string, file: jszip.JSZipObject) => {
      let p: Promise<any>;
      if (file.dir) {
        p = createDir(relPath);
      } else {
