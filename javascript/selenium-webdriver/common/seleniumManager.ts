 ...
 */
export function binaryPaths(args: string[]): BinaryPathsResult {
  ...
  if (spawnResult.status) {
    ...
    if (spawnResult.stdout && spawnResult.stdout.toString()) {
      try {
  ...
  try {
    ...
    throw new Error(`Error executing command for ${smBinary} with ${args}: ${(e as Error).toString()}`);
  }
...
/**
 * Logs the output from Selenium Manager
 * @param output The output from Selenium Manager
 */
