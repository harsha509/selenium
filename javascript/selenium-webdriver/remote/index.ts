...
import { exec, Command as ExecCommand } from '../io/exec';
 ...
 */
type CommandLineFlag = string | Promise<string>;
...
export interface ServiceOptions {
   ...
   */
  args: Array<CommandLineFlag> | Promise<Array<CommandLineFlag>>;
...
export class DriverService {
  ...
  /** @private */
  private loopbackOnly_: boolean;
  /** @private */
  private hostname_?: string;
  /** @private */
  private port_: number | Promise<number>;
  /** @private */
  private args_: Array<CommandLineFlag> | Promise<Array<CommandLineFlag>>;
  /** @private */
  private path_: string;
  /** @private */
  private env_: Record<string, string>;
  /** @private */
  private stdio_: StdIoOptions;
  /** @private */
   ...
   * @param opt_timeoutMs How long to wait, in milliseconds, for the
   *     server to start accepting requests. Defaults to 30 seconds.
   * @return A promise that will resolve to the server's base
  ...
  start(opt_timeoutMs?: number): Promise<string> {
    ...
    let resolveCommand: (command: ExecCommand) => void;
   ...
   * {@link #stop()}.
   * @return A promise that will be resolved when the server has been
   *     stopped.
   */
  kill(): Promise<void> {
  ...
  static Builder = class {
     ...
     * address.
     *
     * @param loopback Whether to use the loopback address.
     * @return A self reference.
     */
     ...
     *
     * @param env The desired environment to use, or `null` if the server should
     *     inherit the current environment.
     * @return A self reference.
     */
    setEnvironment(env: Map<string, string> | Record<string, string> | null): this {
...
/**
 * @param args The command line flags to resolve.
 * @return A promise that will resolve with the resolved flags.
 */
function resolveCommandLineFlags(
  args: Array<CommandLineFlag> | Promise<Array<CommandLineFlag>>
): Promise<string[]> {
  // Resolve the outer array, then the individual flags.
  return Promise.resolve(args).then((args) => Promise.all(args) as Promise<string[]>);
}
...
export class SeleniumServer extends DriverService {
  ...
  constructor(jar: string, opt_options?: SeleniumServerOptions) {
    ...
    let args = Promise.all([port, options.jvmArgs || [], options.args || []]).then((resolved) => {
      ...
      const fullArgsList = jvmArgs.concat('-jar', jar, '-port', port.toString()).concat(args);
...
export class FileDetector extends input.FileDetector {
   ...
   * @override
   */
  handleFile(driver: any, file: string): Promise<string> {
